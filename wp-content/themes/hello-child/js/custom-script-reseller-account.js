...
var trashIconCl = parentElement.querySelector('.placeholder-trash-cl');
...
if (trashIconCl) {
        trashIconCl.style.display = 'block';
    }
    if (colorCode) {
...
function showDeleteAlert(data) {
var deleteAlert = document.getElementById('company_logo_delete_popup_container');
var texts = deleteAlert.querySelectorAll('.deleteText'); // Select all elements with class .deleteText
    var deleteConfirmLogo = deleteAlert.querySelector('#confirmBtnLogo');
    var deleteConfirmCl = deleteAlert.querySelector('#confirmBtnCl');  

    texts.forEach(function(text) {
        text.textContent = data; // Update text content for each element
    });

deleteAlert.style.display = 'flex';

    if(data === 'Company Logo'){
        deleteConfirmLogo.style.display = 'block';
    }else{
        deleteConfirmLogo.style.display = 'none';
    }
    if(data=== 'Color Theme'){
        deleteConfirmCl.style.display = 'block';
    }else{
        deleteConfirmCl.style.display = 'none';
    }
}
...
trashInput.dispatchEvent(event);
...
var trashInput = document.querySelector('#account_company_logo_trash_input');
    ...
    if (fileInput) {
        fileInput.style.backgroundImage = 'url("/wp-content/plugins/thingsatweb/img/smartstoringlogonew.png")';
        fileInput.style.backgroundSize = 'contain';
        fileInput.style.backgroundPosition = 'center';
        fileInput.style.backgroundRepeat = 'no-repeat';
    }
...
function resetColorPicker() {
    var colorPicker = document.getElementById('account_company_theme');
    var defaultColor = '#FFFFFF'; // Set your default color here
    var colorCodeDisplay = colorPicker.parentElement.querySelector('#colorCode');
var colorPickerOverlay = document.getElementById('account_company_theme_overlay');
    var Icon = colorPicker.parentElement.querySelector('.placeholder-image');
    var editIcon = colorPicker.parentElement.querySelector('.placeholder-edit-cl');
    var colorCode = colorPicker.parentElement.querySelector('#colorCode');
    var trashIconCl = colorPicker.parentElement.querySelector('#placeholder-trash-cl');
    
    if (editIcon) {
        editIcon.style.display = 'none';
    }
    if (trashIconCl) {
        trashIconCl.style.display = 'none';
    }
    if (colorCode) {
    ...
    if (colorPicker) {
        ...
        colorCodeDisplay.innerHTML = defaultColor;
        colorPickerOverlay.style.backgroundColor = '#fff'
...
var colorCodeDisplay = document.querySelector('#colorCode');
    ...
    // Set the value to the input with id 'account_company_theme'
    document.getElementById('account_company_theme').value = selectedColor;
    colorCodeDisplay.innerHTML = selectedColor;
var colorPickerOverlay = document.getElementById('account_company_theme_overlay');
...
if (trashIconCl) {
    ...
    function closeColorPopUp() {
        ...
        var colorCodeDisplay = colorPicker.parentElement.querySelector('#colorCode');
        var editIcon = colorPicker.parentElement.querySelector('.placeholder-edit-cl');
var trachIconCl = colorPicker.parentElement.querySelector('.placeholder-trash-cl');
        ...
        if (colorPicker.value !== colorPicker.defaultValue) {
            ...
            editIcon.style.display = 'block';
            trachIconCl.style.display = 'block';
            placeholderImage.style.display = 'none';
        } else {
            editIcon.style.display = 'none';
            trachIconCl.style.display = 'none';
            colorCodeDisplay.style.display = 'none';
            placeholderImage.style.display = 'block';
        }

    }
function triggerFileInputLogo() {
        document.getElementById('account_company_logo').click();
    }
    function triggerFileInputCl() {
        var colorPickerContainer = document.querySelector('.colorPickerContainer');
        colorPickerContainer.style.display = 'flex';
    }
