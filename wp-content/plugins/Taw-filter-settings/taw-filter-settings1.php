...
'Custom Setting',
    ...
    add_submenu_page(
        ...
        'Datasheet Setting', // Submenu page title
        'Datasheet Setting', // Submenu menu title
        'manage_options',   // Capability required to access the submenu
        'datasheet-setting',    // Submenu slug
        'render_datasheetproduct_page' // Callback function to render the submenu page
    );
    add_submenu_page(
        'custom-setting',  // Parent menu slug
        'Adding Category', // Submenu page title
        'Add Category', // Submenu menu title
        'manage_options',   // Capability required to access the submenu
        'category-slug',    // Submenu slug
        'render_category_page' // Callback function to render the submenu page
    );
    ...
    add_submenu_page(
        ...
        'missing_attrivalue_menu_page' // Callback function to render the submenu page
    );
add_submenu_page(
        'custom-setting',  // Parent menu slug
        'Translate attributes', // Submenu page title
        'Translate attributes', // Submenu menu title
        'manage_options',   // Capability required to access the submenu
        'translate-attributes',    // Submenu slug
        'untranslate_attribute_translations' // Callback function to render the submenu page
    );
}
add_action('admin_menu', 'filter_setting_menu');
...
//echo "<script>console.log('Count of results: " . count($attval_results) . "');</script>";

// If you still want to log the results, you can do that as well
//echo "<script>console.log(" . json_encode($attval_results) . ");</script>";

                // Fetch the existing filter settings
                $que = "SELECT MIN(id) AS id, cate_no, attribute FROM taw_filter_setting GROUP BY cate_no;";
                $re_que = $wpdb->get_results($que, ARRAY_A);

$result_scat = "SELECT t.name, t.slug from tsm_terms as t join tsm_term_taxonomy as tax on t.term_id=tax.term_id where tax.taxonomy='product_cat';";
                $results = $wpdb->get_results($result_scat, ARRAY_A);
                $svTerms = array();
                $nonSvTerms = array();
                ...
                foreach ($results as $term) {
                    $termName = $term['slug'];
                    $termssdf = $term['name'];
                    $swediterm_id = $wpdb->get_var("SELECT tsm_icl_translations.element_id
                                        FROM tsm_icl_translations
                                        WHERE tsm_icl_translations.trid = (
                                        SELECT tsm_icl_translations.trid
                                        FROM tsm_terms
                                        JOIN tsm_icl_translations ON tsm_terms.term_id = tsm_icl_translations.element_id
                                        WHERE tsm_terms.slug = '$termName'
                                            AND tsm_icl_translations.element_type LIKE 'tax_product_cat'
                                            AND tsm_icl_translations.language_code = 'en'
                                        )
                                        AND tsm_icl_translations.language_code = 'sv'");
                    $decodedTerm = htmlspecialchars($termssdf);

                    // if (substr($termName, -3) === '-sv') {
                    if (empty($swediterm_id)) {
                        // Term ends with '-sv', so it goes into $svTerms array
                        $svTerms[] = $decodedTerm;
                    } else {
                        // Term does not end with '-sv', so it goes into $nonSvTerms array
                        $nonSvTerms[] = $decodedTerm;
                    }
                }
                // print_r($nonSvTerms);
               // echo "<script>console.log('svterms: " . json_encode($svTerms) . "');</script>";
              //  echo "<script>console.log('enterms: " . json_encode($nonSvTerms) . "');</script>";

                $att_attrib = $wpdb->prepare( "SELECT DISTINCT attribute FROM taw_filter_setting");
                $att_attrib_re = $wpdb->get_results($att_attrib, ARRAY_A);
                $att_attrib_values = [];
                foreach ($att_attrib_re as $item) {
                    $att_attrib_values[] = $item['attribute'];
                }
                ...
                foreach ($attval_results as $attval) {
                   ...
                   // $term_ids_query = $wpdb->prepare("SELECT DISTINCT term_ids FROM taw_article_attributes WHERE attr_id = %s", $attr_id);
                    ...
                    // $att_re = $wpdb->get_results($att_qu, ARRAY_A);
                    // $att_re_values = array_column($att_re, 'att_value');
// if($attr_id == 'pa_loading-height') {
//     echo "<script>console.log('Count of results: " . count($term_ids_results) . "');</script>";
//     echo "<script>console.log('termvalues: " . json_encode($att_re_values) . "');</script>";
// }
                    // Loop through each category
                    foreach ($re_que as $cate) {

                        $att_qu = $wpdb->prepare("SELECT DISTINCT att_value FROM taw_filter_setting WHERE attribute = %s AND cate_no = %s", $attr_id, $cate['cate_no']);
                        $att_re = $wpdb->get_results($att_qu, ARRAY_A);
$att_re_values = array_column($att_re, 'att_value');
                        // Loop through term_ids
                    //     if (!in_array(htmlspecialchars($cate['cate_no']), $svTerms)) {
                    //      $att_qu1 = $wpdb->prepare("SELECT DISTINCT att_value FROM taw_filter_setting WHERE attribute = %s", $attr_id);
                    // $att_re = $wpdb->get_results($att_qu, ARRAY_A);
                    // $att_re_values = array_column($att_re, 'att_value');
                    //     }
                        foreach ($unique_values as $item) {
...
//                             if ($attr_id === 'pa_loading-height') {
//     echo "<script>console.log(" . json_encode($att_re_values) . ");</script>";
//      echo "<script>console.log(" . json_encode($item) . ");</script>";
// }
                           
                            $term_id = $item;
// Check if the insertion has already occurred for this combination
                            $inserted = false;


                                if (!in_array($attr_id, $att_attrib_values)) { 
                                    // $swedishterm_id= $term_id.'-sv';
                                    $swedishterm_id = $wpdb->get_var("SELECT tsm_icl_translations.element_id
                                        FROM tsm_icl_translations
                                        WHERE tsm_icl_translations.trid = (
                                        ...
                                        FROM tsm_terms
                                        JOIN tsm_icl_translations ON tsm_terms.term_id = tsm_icl_translations.element_id
                                        JOIN tsm_term_taxonomy ON tsm_terms.term_id = tsm_term_taxonomy.term_id
                                        WHERE tsm_terms.slug = '$term_id'
                                            AND tsm_icl_translations.element_type LIKE 'tax_pa_%'
                                            AND tsm_icl_translations.language_code = 'en'
                                        AND tsm_term_taxonomy.taxonomy = '$attr_id'
                                        )
                                        AND tsm_icl_translations.language_code = 'sv'");
                                    $swed_termiid = "SELECT tsm_terms.name FROM tsm_terms WHERE tsm_terms.term_id = '$swedishterm_id'";
                                    $sw_termiid = $wpdb->get_results($swed_termiid, ARRAY_A);
                                    ...
                                    foreach ($sw_termiid as $swedattri)
                                     {
                                        $swedishcate_id = $wpdb->get_var("SELECT tsm_icl_translations.element_id
                                        FROM tsm_icl_translations
                                        WHERE tsm_icl_translations.trid = (
                                        SELECT tsm_icl_translations.trid
                                        FROM tsm_terms
                                        JOIN tsm_icl_translations ON tsm_terms.term_id = tsm_icl_translations.element_id
                                        WHERE tsm_terms.slug = '{$cate['cate_no']}'
                                            AND tsm_icl_translations.element_type LIKE 'tax_product_cat'
                                            AND tsm_icl_translations.language_code = 'en'
                                        )
                                        AND tsm_icl_translations.language_code = 'sv'");
                                        //  $swedishcate_id= $cate['cate_no'].'-sv';
                                        $swed_cateiid = "SELECT tsm_terms.name FROM tsm_terms WHERE tsm_terms.term_id = '$swedishcate_id'";
                                        $sw_cateid = $wpdb->get_results($swed_cateiid, ARRAY_A);
                                        ...
                                        if (!$inserted) {
                                            ...
                                            );  // Set the flag to true after the first insertion
                                            $inserted = true; }
...
//                                if($lang == 1){
//                                         //   if ($attr_id === 'pa_loading-height') {
//     echo "<script>console.log(" . json_encode($attributevalue) . ");</script>";
//      echo "<script>console.log(" . ($lang) . ");</script>";
// //}
//                                }
                                    
                                //$attributevalue = (intval($cate['id']) > 20055) ? $swedattri['name'] : $term_id;
                                
                                // $lang = (intval($cate['id']) > 20055) ? 0 : 1;
//if (!$inserted) {
                                // Insert into taw_filter_setting
                                $wpdb->insert(
                                    "taw_filter_setting",
