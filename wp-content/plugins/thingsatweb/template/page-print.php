...
$pageformat = isset($meta['article_price']['product_datasheetpage']) ? $meta['article_price']['product_datasheetpage'] : null;
$isDualImageEnabled = isset($meta['article_price']['product_enable_dual_images_in_datasheet']) ? $meta['article_price']['product_enable_dual_images_in_datasheet'] : false;
...
// Get images - using the same method as your working version
$image1_url = get_the_post_thumbnail_url($product_id);
$fallback_url = THINGSATWEB_BASE . '/img/smartstoring-Fav-Icon-300x300.png';
$image1_url = !empty($image1_url) ? $image1_url : $fallback_url;

// Get gallery image URL if needed
$image2_url = '';
if ($isDualImageEnabled) {
    $gallery_ids = $product->get_gallery_image_ids();
    if (!empty($gallery_ids)) {
        $image2_url = wp_get_attachment_url($gallery_ids[0]);
    }
}

// Convert to base64 - using your proven method
function taw_convert_to_base64($image_url) {
...
$image1_base64 = taw_convert_to_base64($image1_url);
$image2_base64 = taw_convert_to_base64($image2_url);
...
{ 
    ...
    foreach ($formatted_result as $attribute => $att_value) {
        ...
        $label = $label ? $label : $clean_attribute;
            ...
            // Use an associative array to store unique values for the label
    $unique_values = array();
        ...
        // Add the name to the unique values array (only if it hasn't been added before)
        if (!in_array($name, $unique_values)) {
            $unique_values[] = $name;
        }
        }
    
        // Implode the values back
         $cleaned_result[$label] = implode(' | ', $unique_values);
    }
...
}elseif($lang=='sv')
{
    ...
    // Implode the unique values back into a string and add to the cleaned result
    $cleaned_result[$label] = implode(' | ', $unique_values);
    }
    // print_r($cleaned_result);
}
...
<head>
    ...
    { ?>
            ...
            <!-- product image -->
<div style="height: 296px; width: <?php echo $image2_base64 ? '100%' : '500px'; ?>; text-align: center; margin-left: auto; margin-right: auto; margin-top: 10px;">
    <div style="width: 100%; height: 98%; position: relative;">
        
        <?php if ($image1_base64) : ?>
            <div style="<?php echo $image2_base64 ? 'border:1px solid #374151; border-radius:5px; ' : ''; ?> height: 100%; position: absolute; width: <?php echo $image2_base64 ? '48%' : '100%'; ?>;">
                <img src="<?php echo $image1_base64; ?>" alt="Product Image" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: auto; height: auto; max-width: 95%; max-height: 100%; object-fit: contain;">
            </div>
        <?php endif; ?>

        <?php if ($image2_base64) : ?>
            <div style="border:1px solid #374151; right:0; border-radius:5px; height: 100%; position: absolute; width: <?php echo $image2_base64 ? '48%' : '100%'; ?>;">
                <img src="<?php echo $image2_base64; ?>" alt="Product Gallery Image" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: auto; height: auto; max-width: 95%; max-height: 100%; object-fit: contain;">
            </div>
        <?php endif; ?>
            ...
            <!-- product description -->
            <?php
                // Get product description from the database
                $description = $product->get_description();
                $description = preg_replace('/[\x{00A0}\x{202F}\x{200B}\x{FEFF}]/u', ' ', $description);
                // Extract image URL from the content
                preg_match('/<img[^>]+class="([^"]+)"[^>]+src="([^"]+)"[^>]+width="([^"]+)"[^>]+height="([^"]+)"[^>]*>/i', $description, $matches);

                if (!empty($matches[1])) {
                    $image_url =$matches[2]; // Extracted image URL
                    $img_class = !empty($matches[1]) ? trim($matches[1]) : ''; // Extracted class attribute
                    $img_width = !empty($matches[3]) ? trim($matches[3]) : 'auto'; // Extracted width
                    $img_height = !empty($matches[4]) ? trim($matches[4]) : 'auto'; // Extracted height
                    $type = pathinfo($image_url, PATHINFO_EXTENSION);
                    $image_data = file_get_contents($image_url);
                    $base64 = 'data:image/' . $type . ';base64,' . base64_encode($image_data);

                    // Replace <img> tag with Base64 version
                    $description = preg_replace(
                        '/<img[^>]+class="([^"]+)"[^>]+src="([^"]+)"[^>]+width="([^"]+)"[^>]+height="([^"]+)"[^>]*>/i',
                        '<img src="' . $base64 . '" class="' . $img_class . '" width="' . $img_width . '" height="' . $img_height . '" alt="Product Image">',
                        $description
                    );
                }
                // Format description and display
            ...
            <?php if ((strpos($productcategory, "Tailor-made product") === false)) : ?>
                    <?php if($technicalsupportmatchFound) : ?>
                        <?php
                            ...
                            $matchingAttributesCount = 0;

                            foreach ($attributesToDisplay as $attributeKey => $attributeData) { 
                                if (isset($cleaned_result[$attributeKey])) {
                                    $matchingAttributesCount++; // Increase count for each match
                                }
                            }
                        ?>
                        <div style="height:135px; margin-top:-20px; margin-left: -35px; ">
                            <?php if($matchingAttributesCount =='1'){  ?>
                            <div style="width:900px; padding-bottom:0px;">
                            <?php }else{ ?>
            ...
            <div style="height:105px; margin-left: -35px; ">
                    ...
                    if ($weightvolumesupportmatchFound) : ?>
                        <?php
                            ...
                            $current_index = 0; 
                            $max_display_count = 6; // Maximum number of attributes to display in one row
                        ...
                        ?>
                            ...
                            <?php if($matchingAttributesCount == '1'){ ?>
                            <div style="width:500px; max-width:40%; height:10px;">
                            <?php }else{ ?>
                            ...
                            foreach ($attributesToDisplay as $attributeKey => $attributeData) {
                                ...
                                if (isset($cleaned_result[$attributeKey])) {
                                    ...
                                    echo '<div style="width: 1.1%; align-items: center;  display: inline-block; box-sizing: border-box; margin-left:52px; ' . $divHeight . '">';
                        ...
                        <?php
                        if ($weightvolumesupportmatchFound) : ?>
                        <div  style="width:800px; float: left; margin-left:-220px;">
                        <?php
                        elseif (!$weightvolumesupportmatchFound): ?>
                        <div  style="width:800px; float: left; margin-left:26px;">
                        <?php endif; ?>
                        <?php
                            $divHeight = 'height: 90px;';
                            ...
                            $current_index = 0; 
                            $max_display_count = 3; // Maximum number of attributes to display in one row
                            $matchingAttributesCount = 0;
                            ...
                            ?>
                                <?php if($matchingAttributesCount=='1'){ ?>
                                <div style="height: 5px; margin-top: 0px;">
                                <?php }else { ?> 
                                <div style="height: 25px; margin-top: 0px;">
                                <?php } ?>
                                    <span style="width: 400px; font-size: 14px; font-family: 'Poppins', sans-serif; font-weight: bold; margin-left: 25px; margin-bottom: 0; display: block;">
                            ...
                            foreach ($attributesToDisplay as $attributeKey => $attributeData) {
                                ...
                                if (isset($cleaned_result[$attributeKey])) {
                                    echo '<div style="width: 8.5%; align-items: center; padding: 2px; display: inline-block; box-sizing: border-box; margin: 0px 23px; ' . $divHeight . '">';
                                ...
                                ?>
                                    ...
                                    <div style="height: 35px; text-align: center; padding-top:30px; margin-top: 5px;">
                                        <?php
                                        if (!empty($attributeData['image_urls'])) {
                                            ...
                                            // Container for images with text-align center
                                            echo '<div style="text-align: center; font-size: 0;">'; // font-size:0 removes whitespace between inline-block elements
                                            ...
                                            foreach ($sortedImages as $image) {
                                                ...
                                                $image_data = file_get_contents($image_url);
                                                $base64 = 'data:image/' . $type . ';base64,' . base64_encode($image_data);
                                                $width = $image['width'] ?? '30px';
                                                $height = $image['height'] ?? '30px';

                                                echo '<div style="display: inline-block; vertical-align: middle; width: ' . $width . '; height: ' . $height . '; margin: 10px;">';
                                                echo '<img src="' . $base64 . '" alt="Manual" style="max-width: 100%; max-height: 100%; object-fit: contain;" />';
                                                echo '</div>';
                                            ...
                                            if (!$hasImages && !empty($attributeData['image_url'])) {
                                                ...
                                                $image_data = file_get_contents($image_url);
                                                $base64 = 'data:image/' . $type . ';base64,' . base64_encode($image_data);
                                                $width = $attributeData['width'] ?? '30px';
                                                $height = $attributeData['height'] ?? '30px';
                                            ...
                                            <div style="text-align: center;">
                                                <img style="display: inline-block; width: <?php echo $attributeData['width']; ?>;" src="<?php echo $base64; ?>" alt="Manual" />
                                            </div>
                                            <?php
                                        }
                                        ?>
                                    </div>
                                    <span style="position: absolute; font-family: 'Poppins', sans-serif; left: 0; right: 0; text-align: center; font-size: 8px; color: black; font-weight: normal; display: flex; justify-content: center; width: 100%;  height: 5px; margin-top: 5px;">
                                        <?php
                                        $keyvalue = $cleaned_result[$attributeKey];
                                        echo formatAttribute($keyvalue);
                                        ?>
