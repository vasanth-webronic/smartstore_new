<style type="text/css">
    ul#c_uam_caprole_ul_category {
        padding: 10px;
    }

    ul#c_uam_caprole_ul_category li {
        padding: 10px;
    }

        /* For custom_restrictcategory_ls */
    ul.custom_restrictcategory_ls {
        padding: 10px;
    }

    ul.custom_restrictcategory_ls li {
        padding: 15px 30px;
        cursor: pointer;
        position: relative;
    ...
    ul.custom_restrictcategory_ls li.active{
        color: white;
        background: rgb(34, 113, 177);
        border: 2px solid #2271B1;
    }

    ul.custom_restrictcategoryuser_ls {
        padding: 4px;
    }

    ul.custom_restrictcategoryuser_ls li {
        padding: 15px 30px;
        cursor: pointer;
        position: relative;
        background: #FFFFFF 0% 0% no-repeat padding-box;
    ...
    ul.custom_restrictcategoryuser_ls li.active{
        color: white;
        background: rgb(34, 113, 177);
        border: 2px solid #2271B1;
    }
    ...
    #custom-uam-alert-delete-dlg-category {
    display: none;
    position: fixed;
    z-index: 100051;
    top: 50%;
...
.restrict-tooltipcategory-arrow {
    position: absolute;
...
.restrict-tooltipcategoryuser-arrow {
    position: absolute;
    bottom: -5px; /* Arrow height */
    left: 50%;
    transform: translateX(-50%);
...
.aloader {
    ...
    position: absolute; /* Position relative to #loaderhidcategory */
...
<div class="wrap">
    <div class="width:100%;float:left;">
        <p style="margin:0px; width: 50%;color: #000000; font-weight: bold; font-size: 26px;float: left;"><?php esc_html_e('Visible Category', 'custom-uam'); ?> </p>
    ...
    <div style="width: 95%; padding-top: 30px;float: left;">
        ...
        <div style="width: 40%; float: left; position: relative;">
            ...
            <nav aria-label="<?php esc_attr_e('Secondary menu'); ?>" style="display: flex; align-items: center; justify-content: space-between; gap: 0;">
                <div style="display: flex; gap: 0;">
                    <a href="#" id="roles-categorytab" class="navdd-tab navdd-categorytab-active" aria-current="page" style="border-radius: 6px 0 0 6px; font-size: 14px; font-weight: bold;"><?php esc_html_e('Roles'); ?></a>
                    <a href="#" id="users-categorytab" class="navdd-tab" style="border-radius: 0 6px 6px 0; font-size: 14px; font-weight: bold;"><?php esc_html_e('Users'); ?></a>
                </div>
                <input type="text" id="user-categorysearch" placeholder="Search User" style="display: none; width: 50%; background-color: white; border-radius: 8px; border: 1px solid #ccc;">
            </nav>

            <div id="roles-categorycontent">
                <?php
                echo '<ul class="custom_restrictcategory_ls" style="margin-top:18px;">';
                ...
                foreach ($resroles_data as $resrole_slug => $resrole_data) {
                    ...
                    if (strpos($resrole_slug, 'custom_uam') !== false && (!isset($resrole_data['roleissubrole']) || $resrole_data['roleissubrole'] != 1)) {
                        ...
                        echo '<li class="custom-uam-restrictcategoryproduct-li ' . $active_class . '" data-role="' . $resrole_data['name'] . '" data-id="' . $resrole_slug . '">';
                        echo '<span id="clickedrole" class="custom-uam-restrictcategoryproduct-li-title" style="margin-right: auto;">' . $resrole_data['name'] . '</span>';
                        echo '</li>';
            ...
            <div id="users-categorycontent" style="display:none;">
                <div class="scrollable-list-container" style="width: 100%;">
                    <ul class="custom_restrictcategoryuser_ls" style="margin-top:0px;">
                        <?php
                        // Flag to check the first item
                        $is_first = true;
                        foreach ($user_results as $userres) {
                            ...
                            $active_class = $is_first ? ' active' : '';
                            echo '<li style="width: 85%;" class="custom-uam-restrictcategoryproductuser-li' . $active_class . '" data-role="' . $userres['user_login'] . '" data-id="' . $userres['ID'] . '">';
                            echo '<span id="clickedroleuser" class="custom-uam-restrictcategoryproductuser-li-title" style="margin-right: auto;">';
        ...
        <div style="width: 58%; float: left; margin-left: 5px;" id="right-roles-categorycontent">
            ...
            <div style="display: flex; justify-content: flex-end; align-items: center; margin-right: 5px; border-radius: 8px;">
                ...
                <!-- <div id="editrestrictproductcategory" style="background-color: #2271B1; position: relative; border-radius: 8px; display: flex; justify-content: center; padding: 0.25rem; align-items: center; color: #9CA3AF; cursor: pointer; margin-right: 7px;" onmouseover="this.style.backgroundColor='#135e96'" onmouseout="this.style.backgroundColor='#2271B1'">
                    ...
                    <span class="restrict-tooltipcategory" style="display: none;">
                        <span class="restrict-tooltipcategory-arrow"></span>
            ...
            <div style="display: flex; justify-content: flex-start; align-items: center; margin-top: 20px;">
                 <input type="text" id="roleproduct-categorysearch" placeholder="Search Category Name" style="width: 100%; max-width: 250px; background-color: white; border-radius: 4px; border: 1px solid #ccc;">
            </div>            
            <div style="background: #e6e5e5; border-radius: 4px;min-height: 80vh; max-height: auto;">
                <div class="restrictcategory-prod-checkbox-item-container" style="display:none; padding-left: 10px; margin-top: 15px; padding-top: 20px;">
                    <input type="checkbox" id="restrictcategory_select_all"> Select All
                </div>
                <ul id="c_uam_caprole_ul_category" style="display: block;">
                    <!-- Roles-specific restricted products will be listed here -->
                </ul>
                <div id="loaderhidcategory">
                <div class="aloader"></div>
                </div>
                <div id="no-results-category" style="display: none; background-color: rgb(255, 255, 255); padding: 10px; margin: 10px 15px; border-radius: 4px;">
        ...
        <div style="width: 58%; float: left; margin-left: 5px; display:none;" id="right-users-categorycontent" >
            <h3 style="float:left; text-align: left; color: #000000; font-weight: bold; font-size: 20px; margin-left: 10px; margin-top: 5px;">
                <?php echo __("Categories", 'custom-uam'); ?>
            </h3>
            <div style="display: flex; justify-content: flex-end; align-items: center; margin-right: 5px; border-radius: 8px;">
                <div id="deleterestrictcategoryuserproduct" style="background-color: #2271B1; position: relative; border-radius: 8px; display: flex; justify-content: center;padding: 0.25rem; align-items: center; color: #fff; cursor: pointer; margin-right: 7px;" onmouseover="this.style.backgroundColor='#135e96'" onmouseout="this.style.backgroundColor='#2271B1'">
                    <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 12 12">
                        <path fill="currentColor" d="M5 3h2a1 1 0 0 0-2 0M4 3a2 2 0 1 1 4 0h2.5a.5.5 0 0 1 0 1h-.441l-.443 5.17A2 2 0 0 1 7.623 11H4.377a2 2 0 0 1-1.993-1.83L1.941 4H1.5a.5.5 0 0 1 0-1zm3.5 3a.5.5 0 0 0-1 0v2a.5.5 0 0 0 1 0zM5 5.5a.5.5 0 0 1 .5.5v2a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5M3.38 9.085a1 1 0 0 0 .997.915h3.246a1 1 0 0 0 .996-.915L9.055 4h-6.11z" />
                    </svg>        
                </div>
                <!-- <div id="editrestrictuserproduct" style="background-color: #2271B1; position: relative; border-radius: 8px; display: flex; justify-content: center; padding: 0.25rem; align-items: center; color: #9CA3AF; cursor: pointer; margin-right: 7px;" onmouseover="this.style.backgroundColor='#135e96'" onmouseout="this.style.backgroundColor='#2271B1'">
                    ...
                    <span class="restrict-tooltipcategoryuser" style="display: none;">
                        <span class="restrict-tooltipcategoryuser-arrow"></span>
                        Select product to go to edit page
                    </span>
                </div> -->
                <a href="#TB_inline?&width=500&height=300&inlineId=custom-uam-alert-add-edit-dlg" title="<?php esc_html_e('Add Products', 'custom-uam'); ?>" class="thickbox button button-primary" style="border-radius: 8px;">+ <?php esc_html_e('Add', 'custom-uam'); ?></a>
            ...
            <div style="display: flex; justify-content: flex-start; align-items: center; margin-top: 20px;">
                <input type="text" id="userproduct-search-category" placeholder="Search Category Name" style="width: 100%;margin-left: 10px; max-width: 250px; background-color: white; border-radius: 4px; border: 1px solid #ccc;">
            </div>
            <div  style="background: #e6e5e5;border-radius: 4px;margin-left: 10px;min-height: 80vh; max-height: auto;padding-top: 5px;margin-top: 13px;" >
                <div class="restrictcategoryuser-prod-checkbox-item-container" style="display:none; padding-left: 10px; margin-top: 15px; padding-top: 20px;">
                    <input type="checkbox" id="restrictcategoryuser_select_all"> Select All
                </div>
                <ul id="c_uam_capuser_ul_category" style="display: block;">
                    <!-- User-specific restricted products will be listed here -->
                </ul>
                <div id="loaderusercategory">
                <div class="aloaderusercategory"></div>
                </div>
                <div id="nouser-results-category" style=" display: none;  background-color: rgb(255, 255, 255);padding: 10px;margin: 10px 15px;border-radius: 4px;">
                    <span style="color: red; font-weight: medium;">No results found</span>
                </div>
            </div>
            <!-- <div id="no-results-category" style="display: none; color: red; text-align: center;">No results found</div> -->
            <div style="text-align: center;">
    ...
    <div id="custom-uam-alert-add-edit-dlg" style="display:none">
        <div style="text-align: center;">
            ...
            <div style="margin-bottom: 10px;">
                <form method="post" action="<?php echo admin_url('admin-ajax.php'); ?>" id="custom_uam_save_restrictcategory_artno">
                    <input style="width: 84%; margin-bottom: 10px; margin-top: 30px; margin-left: 0px; border-radius: 4px;" type="text" placeholder="Enter Category Name" name="restrict_artno" id="custom-uam-input-restrictcategory-artno">
                    <button type="button" id="restrictcategory-add-artno-btn" class="button button-primary" style="padding-left: 20px; padding-right: 20px; font-size: 14px; width: 15%; margin-bottom: 10px; border-radius: 6px; margin-top: 30px;"><?php echo __('Add', 'custom_uam'); ?></button>
                    <div id="restrictcategory-search-results" style="width: 100%; display: none; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1); cursor: pointer;">
                        <div class="restrictcategory-art-search-results" style="text-align:start; padding: 0.5rem; color: #000; background-color: #f8fafc; margin-top: -10px; z-index: 999999999; position: absolute; max-height: 14rem; overflow-y: auto; border: 1px solid #e2e8f0; width: 76%; scrollbar-width: thin; scrollbar-thumb-radius: 4px; scrollbar-color: gray; scrollbar-track-color: #ccc;">
                            <!-- Search results will be appended here -->
                        </div>
                    </div>
                    <ul id="restrictcategory-entered-artnos" style="list-style-type: none; padding: 0;"></ul>
                    <input type="hidden" name="restrictcategory-all_artnos" id="restrictcategory-all-artnos" value="">
                    <input type="hidden" name="action" value="custom_uam_save_restrictcategory_artno" />
    ...
    <div id="custom-uam-alert-delete-dlg-category" style="display:none;">
        <div style="text-align: center;">
            <p id="custom-uam-alert-msg-category" style="font-weight:bold; width:330px;"></p>
            <div style="margin-bottom: 10px;">
                <button type="button" id="confirm-remove-categorybtn" class="button button-primary" style="border:#D5352C;background: #D5352C 0% 0% no-repeat padding-box; padding-left: 20px; padding-right: 20px; font-size: 14px; width: 35%; margin-bottom: 10px; border-radius: 6px;"><?php echo __('Remove', 'custom_uam'); ?></button>
                <button type="button" id="cancel-remove-categorybtn" class="button" style="color:black; border:#F0F0F1; background: #F0F0F1 0% 0% no-repeat padding-box; padding-left: 20px; padding-right: 20px; font-size: 14px; width: 35%; margin-bottom: 10px; border-radius: 6px;"><?php echo __('Cancel', 'custom_uam'); ?></button>
            </div>
        </div>
    </div>
    <div id="deleteovercategory"  style="display:none;"></div>
</div>
