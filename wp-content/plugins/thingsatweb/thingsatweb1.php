...
function sync_woocommercedata($page_num, $cur_lang)
{
    ...
    foreach ($res as $r) {
        ...
        if (!empty($category)) {
            ...
            $child = str_replace('-sv', '', $childquery);
            $parent = str_replace('-sv', '', $parentquery);

            $uniqucode = "$sku::$lang_code::$child>>$parent";
    ...
    function set_product($id, $post)
    {
        ...
        if ($post->post_status === 'publish' && $post->post_type === 'product') {
            ...
            if (!empty($category)) {
                ...
                $uniqucode="$art_no::$lang::$child>>$parent";
...
?>
    <div class="flex justify-center lg:justify-start mt-6">
        ...
        <a href="https://smartstoring.eu/downloads/<?php echo $product->get_id() ?>/datasheet?print=generate_datasheet">          
...
function spareparts($post_id, $post) {
    ...
    $img_pointer_info = isset($_POST['_wpii_prod_img_pointer_info']) ? $_POST['_wpii_prod_img_pointer_info'] : '';

    $taw_prod_opt = isset($_POST['taw_prod_opt']) ? $_POST['taw_prod_opt'] : '';
    // print_r($taw_prod_opt);
    // exit;


    // Save the meta information for the original post
    update_post_meta($post_id, '_wpii_prod_img_pointer_info', $img_pointer_info);
    update_post_meta($post_id, 'taw_prod_opt', $taw_prod_opt);
