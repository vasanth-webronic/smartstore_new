 ...
 * @author     Webronic
 * @copyright  2020 Webronic
 * @license    https://webronic.com
 * @link       link(https://webronic.com)
 * @since      1.0.0
 * php version 7.4.0
 */
namespace thingsatweb\Widgets;
 ...
 * @since 1.0.0
 */
class Widget_Custom_Prod_Family extends Widget_Base {
	...
	public function get_name() {
		return 'custom-family';//image-box';
	}
	...
	public function get_title() {
		return esc_html__( 'Custom Product Family', 'thingsatweb' );
	}
	...
	public function get_keywords() {
		return [ 'product', 'family', 'custom', 'box' ];
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'price_text',
			[
				'label' => esc_html__( 'Price From', 'elementor' ),
				'type' => Controls_Manager::TEXT,
				'dynamic' => [
					'active' => true,
				],
				'default' => esc_html__( '0.00', 'elementor' ),
				'placeholder' => esc_html__( 'Price From', 'elementor' ),
				'label_block' => true,
			]
		);
		...
		//get list from
		global $wpdb;	
		$q="SELECT name FROM `taw_product_lookup` where type='family' and status='active'";
		...
		$this->add_control(
			'family_text',
			[
				'label' => esc_html__( 'Select Family', 'elementor' ),
	...
	protected function render() {	
		...
		if(is_array($settings['family_text'])){
			$settings['family_text']='';
		}
		...
		if ( ! empty( $settings['image']['url'] ) ) {
			...
			if( !empty($settings['family_text'])){
				$url= "/".$post->post_name."/".str_replace(" ","-",$settings['family_text'])."/";
		...
		if ( $has_content ) {
			...
			if ( ! Utils::is_empty( $settings['price_text'] ) ) {
				$this->add_render_attribute( 'price_text', 'class', 'elementor-image-box-description' );

				$this->add_inline_editing_attributes( 'price_text' );

				$html .= sprintf( '<p %1$s>From %2$s kr</p>', $this->get_render_attribute_string( 'price_text' ), $settings['price_text'] );
			}
	...
	protected function content_template() {
		...
		var url="#"			
		if(!Array.isArray(settings.family_text)){				
			var txt=settings.family_text.replace(' ','-');
			url='/design-your-door/'+txt+'/';
		}
		...
		if ( hasContent ) {
			...
			if ( settings.price_text ) {
				view.addRenderAttribute( 'price_text', 'class', 'elementor-image-box-description' );

				view.addInlineEditingAttributes( 'price_text' );

				html += '<p>From <span ' + view.getRenderAttributeString( 'price_text' ) + '>' + settings.price_text + '</span> kr</p>';
			}
